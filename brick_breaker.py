...
class Game(tk.Frame):
    ...
    def init_game(self):
        ...
        self.display_ball()
        self.text = self.draw_text(
            self.width/2, self.height/2, 'Press S to start game')
...
#activating the letter "s" as a game startt button
    ...
    def game_loop(self):
        ...
        if num_bricks == 0:
            ...
            self.draw_text(self.width/2, self.height/2, "flawless victory, you win!")
        elif self.ball.position()[3] >= self.height:
            self.ball.speed = None
            self.lives -= 1
            if self.lives == 0:
                self.draw_text(self.width/2, self.height /
                               2, "hahaha nice try, gameover!")
            else:
